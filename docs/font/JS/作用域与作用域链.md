...
![1558861220481](../../.vuepress/public/1558861220481.png)
...
在fn函数中，取自由变量x的值时，要到哪个作用域中取？——:yum:**要到创建fn函数的那个作用域中取**，**无论fn函数将在哪里调用**。
...
作用域和执行上下文之间最大的区别是： :yum:**执行上下文在运行时确定，随时可能改变；作用域在定义时就确定，并且不会改变**。
