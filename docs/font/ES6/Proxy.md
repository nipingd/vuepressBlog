...
:star:一个技巧是将 Proxy 对象，设置到`object.proxy`属性，从而可以在`object`对象上调用。

```javascript
var object = { proxy: new Proxy(target, handler) };
```

:ox:Proxy 实例也可以作为其他对象的原型对象。
...
:star2:下面的例子使用`get`拦截，实现数组读取负数的索引。
...
:o:注意，如果目标对象自身的某个属性，不可写且不可配置，那么`set`方法将不起作用。

:o:注意，严格模式下，`set`代理如果没有返回`true`，就会报错。
...
:ox:如果原对象不可配置或者禁止扩展，这时`has`拦截会报错。

:fire:值得注意的是，`has`方法拦截的是`HasProperty`操作，而不是`HasOwnProperty`操作，即`has`方法不判断一个属性是对象自身的属性，还是继承的属性。
...
:fire:注意，使用`Object.keys`方法时，有三类属性会被`ownKeys`方法自动过滤，不会返回。
...
:star2:上面代码中，`getDate`方法只能在`Date`对象实例上面拿到，如果`this`不是`Date`对象实例就会报错。这时，`this`绑定原始对象，就可以解决这个问题。
